...
typedef NS_ENUM(NSInteger, V2NIMTeamJoinActionType) {
    ...
    V2NIM_TEAM_JOIN_ACTION_TYPE_ACCEPT_APPLICATION  = 4,    ///< 自己收到管理员同意入群申请
    V2NIM_TEAM_JOIN_ACTION_TYPE_ACCEPT_INVITATION   =   5,  ///< 自己（邀请人）收到被邀请人同意入群邀请
};
...
/// 成员入群操作类型
typedef NS_ENUM(NSInteger, V2NIMTeamJoinActionTeamType) {
    V2NIM_TEAM_JOIN_ACTION_TEAM_TYPE_NORMAL              = 1,  ///< 高级群
    V2NIM_TEAM_JOIN_ACTION_TEAM_TYPE_SUPER             = 2,  ///< 超大群
    V2NIM_TEAM_JOIN_ACTION_TEAM_TYPE_ALL             = 3,  ///< 所有
};
 ...
 *  @param failure 失败回调
 */
- (void)getTeamInfoFromCloud:(NSString *)teamId
                    teamType:(V2NIMTeamType)teamType
                     success:(nullable V2NIMTeamInfoCallback)success
                     failure:(nullable V2NIMFailureCallback)failure;
 ...
 *  @param failure 失败回调
 */
- (void)getManagerTeamList:(NSArray<NSNumber *> *)teamTypes
                   success:(nullable V2NIMTeamInfoListCallback)success
                   failure:(nullable V2NIMFailureCallback)failure;
 ...
 *  @param failure  请求失败回调
 */
- (void)clearAllTeamJoinActionInfoEx:(V2NIMTeamClearJoinActionInfoOption *)option
                    success:(nullable V2NIMSuccessCallback)success
                       failure:(nullable V2NIMFailureCallback)failure;
...
/// 目标账号
///     仅邀请时有值
@property(nonatomic, copy, nullable) NSString *targetAccountId;
/// 服务端ID
///     本地历史数据为null
///     为null删记录只删除本地即可
...
/// 附言历史
///     合并最近10次的记录
@property (nonatomic, copy, readonly) NSArray<V2NIMPostscript *> *postscriptHistory;
@end
...
/// 清空群申请/邀请记录的选项
@interface V2NIMTeamClearJoinActionInfoOption : NSObject <NSCopying>
/// 时间戳，默认当前时间戳，会删除这个时间之前的所有记录
@property (nonatomic, assign) NSTimeInterval timestamp;
/// 群类型，默认全部 (V2NIM_TEAM_JOIN_ACTION_TEAM_TYPE_ALL)
@property (nonatomic, assign) V2NIMTeamJoinActionTeamType type;
@end
