...
/// 发送端类型
@property(nonatomic,assign,readonly) V2NIMLoginClientType fromClientType;
@end
...
/// 清空删除本地消息参数
@interface V2NIMClearLocalMessageParams : NSObject<NSCopying>

/**
 * 清理指定时间之前的所有消息。默认值为0 ， 表示当前时间，表示清理当前时间之前的所有消息。单位：毫秒
 */
@property(nonatomic,assign) long long anchorTime;

/**
 * 是否同步删除会话入口，主要删除本地会话。默认YES
 * YES：表示会同步删除时间段内相关的会话入口
 * NO：会话入口会保留
 */
